...
public class VariablesQuery {
	...
	public VariablesQuery(KieServerRegistry kieServerRegistry, QueryService queryService) {
	...
	public Response tasksVariables(@Context HttpHeaders headers, @QueryParam("vars") List<String> vars) {
		...
		try {
			...
			for (String var : vars) {
				variableColumns += String.format(", MAX ( CASE V.VARIABLEINSTANCEID WHEN '%s' THEN V.VALUE END ) AS VAR_%s", var, var);
